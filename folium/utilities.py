...
def mercator_transform(
    ...
    def mercator(x):
        return cmath.asinh(cmath.tan(x * cmath.pi / 180.0)) * 180.0 / cmath.pi

    def atleast_3d(*args):
        """
        Принимает один или несколько массивов и возвращает их,
        упакованные в трёхмерные массивы, даже если они являются скалярами.
        """
        out_arrays = []
        for arr in args:
            # Если arr является списком или кортежем, преобразуем его в трёхмерный список
            if isinstance(arr, (list, tuple)):
                arr = [[x] for x in arr]
            # Если arr уже является двумерным списком, добавляем его как есть
            elif isinstance(arr, (list, tuple)) and all(isinstance(x, (list, tuple)) for x in arr):
                arr = [arr]
            # Если arr является скаляром или одномерным списком, преобразуем его в трёхмерный список
            else:
                arr = [[arr]]
            out_arrays.append(arr)
        # Если передан один массив, возвращаем его, иначе возвращаем список массивов
        return out_arrays[0] if len(out_arrays) == 1 else out_arrays

    def shape(arr):
        """
        Возвращает форму (shape) трёхмерного списка.
        """
        if isinstance(arr, list) and all(isinstance(x, list) for x in arr) and all(
                isinstance(y, list) for x in arr for y in x):
            # Получаем количество элементов по каждой оси
            dim1 = len(arr)
            dim2 = len(arr[0])
            dim3 = len(arr[0][0])
    ...
    array = deep_copy(atleast_3d(data))
    height, width, nblayers = shape(array)
    ...
    def linspace(start, stop, num=50, endpoint=True):
        """
        Возвращает массив чисел с равномерно разделенными значениями
    ...
    lats = add(multiply(linspace(0.5 / height, 1.0 - 0.5 / height, height), lat_max - lat_min), lat_min)
    latslats = add(multiply(linspace(0.5 / height_out, 1.0 - 0.5 / height_out, height_out),
                            (mercator(lat_max) - mercator(lat_min))), mercator(lat_min))
    ...
    def zeros(shape, dtype=float):
        ...
        if isinstance(shape, int):
            ...
            raise ValueError("Input shape must be an integer or a tuple of integers.")

    out = zeros((height_out, width, nblayers))

    def interp_linear(x, xp, fp):
        """
        Линейная интерполяция значений x на основе заданных точек xp и соответствующих значений fp.
        """
        if len(xp) != len(fp):
            raise ValueError("Lengths of xp and fp must be equal.")

        def _interpolate(x):
            i = 0
            while xp[i] < x and i < len(xp) - 1:
                i += 1
            if xp[i] == x:
                return fp[i]
            else:
        ...
        return [_interpolate(x_val) for x_val in x]

    for i in range(width):
        for j in range(nblayers):
            interp_values = interp_linear(latslats, [mercator(lat) for lat in lats], [row[i][j] for row in array])
            for k, val in enumerate(interp_values):
                out[k][i] = val
