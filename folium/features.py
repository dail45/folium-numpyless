    ...
    def __init__(
        ...
        **kwargs,
        ...
        def histogram(values, bins):
            """
            Вычисляет гистограмму значений `values` с использованием заданных интервалов `bins`.
            """
            hist = [0] * (len(bins) - 1)
            for val in values:
        ...
        if color_data is not None and key_on is not None:
            ...
            real_values = [val for val in values_list if not isinstance(val, float) or val == val]
            if use_jenks:
                ...
                bin_edges = list(jenks_breaks(real_values, bins))
            else:
                _, bin_edges = histogram(real_values, bins=bins)

            def any_outside_range(values, min_val, max_val):
                """
                Проверяет, есть ли хотя бы одно значение в списке `values`,
                которое находится за пределами заданного диапазона [`min_val`, `max_val`].
                """
                for val in values:
            ...
            if any_outside_range(real_values, bins_min, bins_max):
                raise ValueError(
            ...
            def nextafter(x, y):
                """
                Возвращает следующее (ближайшее к y) значащее число после x в направлении к y.
                """
                if x == y:
                    return x
                elif x < y:
            ...
            bin_edges[-1] = nextafter(bin_edges[-1], (1 if increasing else -1) * cmath.inf)
            ...
            def color_scale_fun(x):
                ...
                if value_of_x != value_of_x:
                    return nan_fill_color, nan_fill_opacity

                def digitize(value, bins, right=False):
                    """
                    Определяет индекс корзины, в которую попадает значение value,
                    используя заданные границы интервалов bins.
                    """
                    if right:
                ...
                color_idx = digitize(value_of_x, bin_edges, right=False) - 1
